...
async function submitContactForm(url: string, data: ContactData): Promise<string> {
  ...
  try {
    ...
    await formLocator.waitFor({ state: 'visible', timeout: 1000 });
    ...
    try {
      ...
      for (const checkbox of checkboxes) {
        ...
        try {
          ...
          console.warn("Could not check a checkbox.");
        }
      }
 ...
 try {
    const submitButton = page.locator(
      'button[type="submit"], input[type="submit"], button[aria-label="送信"], button[alt="送信"], input[aria-label="送信"], input[alt="送信"]'
    );
    ...
    if (!submitButton.isVisible)
      await submitButton.last().scrollIntoViewIfNeeded();
...
(async () => {
  ...
  const status = await submitContactForm(url, contactData);
  parentPort?.postMessage({ url, status });
})();
